...
# Перехватчик критических ошибок: если что-то пойдет не так,
# игра не вылетит втихую, а запишет лог и покажет его
def run_game():
    import pygame
    import random
    import math
    ...
    # 1. Безопасная инициализация звука
    audio_enabled = True
    try:
    ...
    # 2. Безопасные шрифты для Android (pygame.font.Font(None) вместо SysFont)
    f_title = pygame.font.Font(None, int(WIDTH * 0.11))
    f_sub = pygame.font.Font(None, int(WIDTH * 0.040))
    f_ui = pygame.font.Font(None, int(WIDTH * 0.052))
    f_card = pygame.font.Font(None, int(WIDTH * 0.060))

    # 3. Безопасное сохранение прогресса в Android App Sandbox
    save_dir = os.environ.get("ANDROID_PRIVATE", os.path.expanduser("~"))
    SAVE_FILE = os.path.join(save_dir, "snakehunt_save.json")
    ...
    # 4. Аудиосистема с защитой от сбоев
    music_volume = 0.5
    music_channel = None
    sfx_channel = None
    snd_eat = snd_bonus = snd_crash = snd_win = None
    melody_sounds = []
    ...
    if audio_enabled:
        try:
            ...
            for f, d in notes:
                s = generate_sine_wave(f, d, 0.3)
                if s: melody_sounds.append(s)
        except Exception:
            audio_enabled = False
    ...
    def update_music(now):
        nonlocal current_note_idx, last_note_time
        if audio_enabled and melody_sounds and music_volume > 0 and (now - last_note_time >= 220):
            try:
    ...
    def play_sfx(sound):
        if audio_enabled and sound and music_volume > 0:
            try:
    ...
    def generate_level_walls(lvl):
        ...
        if lvl == 2:
            for dx in (-3, 3):
                for dy in (-3, 3): walls[(mx + dx, my + dy)] = "SPIKE"
        elif lvl == 3:
    ...
    def start_game(mode="CAMPAIGN", lvl=1):
        nonlocal bonus_item, freeze_end_time, game_mode
        game_mode = mode
        mx, my = GRID_X // 2, GRID_Y // 2
        snake = [(mx, my), (mx - 1, my), (mx - 2, my)]
        walls = generate_level_walls(lvl) if mode == "CAMPAIGN" else {}
...
if __name__ == "__main__":
    try:
        ...
        try:
            import pygame
            pygame.init()
            s = pygame.display.set_mode((720, 1280))
            f = pygame.font.Font(None, 24)
            while True:
                s.fill((20, 0, 0))
                y = 20
                for line in err_msg.split("\n"):
                    txt = f.render(line[:60], True, (255, 100, 100))
                    s.blit(txt, (20, y))
                    y += 26
                pygame.display.flip()
